...
public class Deleteempleado extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Deleteempleado at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
     ...
     if(empleado != null){
          mensaje = "Ha sido borrado el empleado con el nombre de " + empleado.getNombre();    
     empleados.remove(empleado);
request.setAttribute("mensaje", mensaje);
request.getRequestDispatcher("/admin/listaempleados").forward(request, response);
        
     }else{
          mensaje = "No se ha encontrado el usuario.Intenta nuevamente"; 
request.setAttribute("mensaje", mensaje);
request.getRequestDispatcher("/admin/listaempleados").forward(request, response);         
     }
